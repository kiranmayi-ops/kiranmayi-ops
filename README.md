<h2> Hi ,I'm Akella Kiranmayi</h2>
I'm a B.Tech First Year Computer Science Student who loves learning programming and exploring new technologies.  
This repository contains my practice codes, mini-projects, and experiments as I learn different programming languages and concepts.
<hr> 

<h2>About me</h2>
<h3>Currently learning</h3>
 <ul>
   <li>C Programming</li>
   <li>Data Structures</li>
   <li>Web Development (HTML, CSS, JavaScript)</li>
  <li>python</li>
 </ul>
 <hr>
 <h3>What I Enjoy</h3>
 <ul>
   <li>Writing clean, logical, and efficient code</li>
   <li>Learning new programming concepts everyday</li>
   <li>Designing simple and user friendly web pages </li>
 </ul>
 <hr>
 <h2>Goals</h2>
 <ul>
   <li>Master core computer science concepts</li>
   <li>Build real-world applications</li>
   <li>Grow as full-stack developer in the future </li>
 </ul>
